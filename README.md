...
gskill sync                               # reconcile disk to the manifest (--prune removes orphans)
...
`gskill sync`, or add them imperatively:
...
| `sync` | Reconcile disk to the manifest's desired state (`--prune` removes orphans). |
...
| `status` | Per-skill, per-agent install mode and health. |
