...
gskill project sync                               # reconcile disk to the manifest (--prune removes orphans)
...
via three layers (all but the manifest/lock are gitignored and regenerated by
`gskill project sync`):
...
no duplicate content. Declare the desired agents in `gskill.toml` and run
...
| **PROJECT** | |
| `project sync` | Reconcile disk to the manifest's desired state (`--prune` removes orphans). |
| `project repair` | Re-materialize broken installs; clean orphaned staging. |
| `project lock` | Recompute the lock from the manifest without bumping pins. |
...
| `project diff` | Show manifest/lock/disk differences. |
| **MORE** | |
